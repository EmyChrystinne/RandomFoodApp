...
const FilteringOptions = () => {
  ...
  const categories = [
    ...
    { id: 2, name: 'Preço Médio', options: ['Até R$50', 'R$50 - R$100', 'R$100 - R$200', 'Acima de R$200'] },
    { id: 3, name: 'Localização', options: ['Parte Baixa', 'Parte Alta'] },
    { id: 4, name: 'Categorias', options: ['Boteco', 'Cafeteria', 'Fast-food', 'Lanchonete', 'Padaria', 'Restaurante', 'Rodízio', 'Sorveteria', 'Outro'] }
  ];
  ...
  return (
    <div className='FilteringOptions'>
      ...
      {categories.map(category => ( // Mapeia as categorias
        <div key={category.id} className='categories'> 
          <div className="category"> {/* Div para cada categoria */}
            <h3>{category.name}</h3>
            {/* Mapeia as opções da categoria */}
            <div className="options">
            {category.options.map(option => ( // Mapeia as opções da categoria atual
              <div key={option} className="option"> {/* Div para cada opção */}
                <label className='checkbox-input'> 
                  <input
                    type="checkbox"
                    checked={selectedOptions[category.name]?.[option]} // Verifica se a opção está selecionada
                    onChange={() => handleOptionSelect(category.name, option)} // Manipula a seleção da opção
                    className='checkbox'
                  />
                  <span>{option}</span>
                </label>
      ...
      <button className='Surprise'>Mostre-me as opções</button> {/* Botão para mostrar as opções selecionadas */}
    </div>
  );
}
